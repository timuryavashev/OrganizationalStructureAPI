...
Переменная `DATABASE_HOST` должна иметь значение `db`, переменная `DATABASE_PORT=5432`, `DEBUG=True`  
Остальные переменные могут иметь произвольный вид.

## 4. Создание и удаление docker
